...
test("payment proof is limited to Admin and Finance regardless of program membership", () => {
  for (const user of [{ id: "lead-1", role: "LEAD" }, { id: "member-1", role: "MEMBER" }] as const) {
...
test("members can manage only their own uploads in active programs", () => {
  const owner = { id: "member-1", role: "MEMBER" } as const;
  const other = { id: "member-3", role: "MEMBER" } as const;
  const inactive = { id: "member-2", role: "MEMBER" } as const;
