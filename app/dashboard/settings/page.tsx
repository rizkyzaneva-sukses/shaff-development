...
type Role = "ADMIN" | "LEAD" | "FINANCE" | "CMO" | "COO";
...
const roleLabels: Record<Role, string> = { ADMIN: "Admin", LEAD: "Lead", FINANCE: "Finance", CMO: "CMO", COO: "COO" };
...
export default function SettingsPage() {
  ...
  const [inviteForm, setInviteForm] = useState({ name: "", email: "", jobTitle: "", role: "CMO" as Role });
  ...
  const submitInvite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setInviteLink(`${window.location.origin}${result.activationUrl}`); setInviteForm({ name: "", email: "", jobTitle: "", role: "CMO" }); notify("Undangan dibuat; salin link aktivasi untuk anggota");
  ...
  return <div className="ops-page">
    ...
    <section className="mt-6 flex gap-5 overflow-x-auto border-b border-[var(--border)]"><TabButton active={tab === "organization"} onClick={() => setTab("organization")}>Profil organisasi</TabButton><TabButton active={tab === "team"} onClick={() => setTab("team")} count={team.length || undefined}>Anggota tim</TabButton><TabButton active={tab === "audit"} onClick={() => setTab("audit")}>Audit log</TabButton>{process.env.NODE_ENV !== "production" && <TabButton active={tab === "demo"} onClick={() => setTab("demo")}>Data demo / reset</TabButton>}</section>
    ...
    {process.env.NODE_ENV !== "production" && tab === "demo" && <section className="surface mt-6 p-5 sm:p-7">
