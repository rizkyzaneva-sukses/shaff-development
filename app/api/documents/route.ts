...
export async function GET(request: Request) {
  try {
    ...
    const where: Prisma.DocumentWhereInput = {
      ...
      ...(user.role === "LEAD" ? { program: { client: { leadId: user.id } } } : isExecutor(user.role) ? { program: { members: { some: { userId: user.id, isActive: true } } } } : {}),
    };
