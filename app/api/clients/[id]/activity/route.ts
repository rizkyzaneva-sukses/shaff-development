...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const client = await prisma.client.findFirst({ where: { id, ...(user.role === "ADMIN" || user.role === "FINANCE" ? {} : user.role === "LEAD" ? { leadId: user.id } : { programs: { some: { members: { some: { userId: user.id, isActive: true } } } } }) }, select: { id: true, programs: { where: user.role === "MEMBER" ? { members: { some: { userId: user.id, isActive: true } } } : undefined, select: { id: true, tasks: { select: { id: true } }, meetings: { select: { id: true } }, documents: { select: { id: true } }, invoices: { select: { id: true } } } } } });
